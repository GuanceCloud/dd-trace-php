...
define FPM_OPTS
	-a $(1) -n $(PACKAGE_NAME) -m dev@datadoghq.com --license "BSD 3-Clause License" --version $(VERSION) \
		--provides $(PACKAGE_NAME) --vendor DataDog  --url "https://docs.datadoghq.com/tracing/setup/php/" --no-depends --force \
...
dbgsym.tar.gz: $(PACKAGES_BUILD_DIR)
	$(if $(DDTRACE_MAKE_PACKAGES_ASAN), , if [ -d ./extensions_x86_64_debugsymbols ]; then tar -zcf $(PACKAGES_BUILD_DIR)/dd-library-php-$(VERSION)_windows_debugsymbols.tar.gz ./extensions_x86_64_debugsymbols --owner=0 --group=0; fi)
...
PHPUNIT_OPTS ?=
PHPUNIT_JUNIT ?=
PHPUNIT = $(TESTS_ROOT)/vendor/bin/phpunit $(PHPUNIT_OPTS) $(if $(PHPUNIT_JUNIT),--log-junit $(PHPUNIT_JUNIT) --dont-report-useless-tests) --config=$(TESTS_ROOT)/phpunit.xml
PHPUNIT_COVERAGE ?=
PHPBENCH_OPTS ?=
